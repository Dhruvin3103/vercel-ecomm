...
class UpdateDeleteProductCartAPI(GenericAPIView):
    ...
    def patch(self,request):
        serializer = self.serializer_class(Product_cart.objects.get(user = request.user.id,product_by_size=request.data['product_by_size']),data=request.data,partial =True)
...
class ProductCartAPI(ListCreateAPIView,ProductCartMixin,UpdateModelMixin):
    ...
    def perform_create(self, serializer):
        user = self.request.user
        product_by_size = serializer.validated_data['product_by_size']
        count = serializer.validated_data['count']
        lookup_field = 'product'

        try:
            # Check if the product already exists in the user's cart
            cart_item = Product_cart.objects.get(user=user, product_by_size=product_by_size)
